                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    { // nearest neighbor (best match)
        ...
        cout << "Matched keypoints are = " << matches.size() << endl;
    }
...
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, std::string detectorType, bool bVis) {
    ...
    else if (detectorType.compare("HARRIS") == 0)
    {
        ...
        {
            bool bNMS = true; // Boolean to perform Non-maxima supression.
            int threshold = 30; // Threshold for checking the darker/lighter pixels around the target pixel.

            cv::FastFeatureDetector::DetectorType type = cv::FastFeatureDetector::TYPE_9_16; //selecting the type of detector.
            detector = cv::FastFeatureDetector::create(threshold, bNMS, type); // creating detector.
...
//     cv::FastFeatureDetector::DetectorType type = cv::FastFeatureDetector::TYPE_9_16; //selecting the type of detector.
//     cv::Ptr<cv::FeatureDetector> detector = cv::FastFeatureDetector::create(threshold, bNMS, type); // creating detector.
